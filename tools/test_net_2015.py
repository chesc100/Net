...
def draw_boxes(img,image_name,boxes,scale):
    ...
    with open('data/results/' + 'res_{}.txt'.format(base_name.split('.')[0]), 'w') as f:
        for box in boxes:
            ...
            line = ','.join([str(min_x),str(min_y),str(max_x),str(min_y),str(max_x),str(max_y),str(min_x),str(max_y)])+'\r\n'
            f.write(line)
...
if __name__ == '__main__':
    ...
    im_names = glob.glob(os.path.join(cfg.DATA_DIR, 'eval/2015', '*.png')) + \
               glob.glob(os.path.join(cfg.DATA_DIR, 'eval/2015', '*.jpg'))
