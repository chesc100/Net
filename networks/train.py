...
from utils.roi_data_layer import roidb as rdl_roidb
from utils.roi_data_layer.layer import RoIDataLayer
...
class SolverWrapper(object):
    def __init__(self, sess, network, imdb, roidb, output_dir, logdir, pretrained_model=None):
        ...
        self.writer = tf.summary.FileWriter(logdir=logdir, graph=tf.get_default_graph(), flush_secs=5)
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        elif cfg.TRAIN.SOLVER == 'GD':
            opt = tf.train.GradientDescentOptimizer(cfg.TRAIN.LEARNING_RATE)
        else:
...
#                self.net.keep_prob: 0.5,
            ...
            if (iter+1) % (cfg.TRAIN.DISPLAY) == 0:
                print('iter: %d / %d, total loss: %.4f, model loss: %.4f, rpn_loss_cls: %.4f, rpn_loss_box: %.4f, lr: %f'%\
                      (iter+1, max_iters, total_loss_val, model_loss_val, rpn_loss_cls_val, rpn_loss_box_val, lr.eval()))
                print('speed: {:.3f}s / iter'.format(_diff_time))

                if float(total_loss_val) < min_loss:
                    print('\ntotal loss: %.4f' % (total_loss_val))
                    min_loss = float(total_loss_val)
