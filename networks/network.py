...
from utils.rpn.anchor_target_layer import anchor_target_layer as anchor_target_layer_py
from utils.rpn.proposal_layer import proposal_layer as proposal_layer_py
...
class Network(object):
    ...
    @layer
    def rpn_fc(self, input, d_i, d_o, name, trainable=True):
        with tf.variable_scope(name) as scope:
            shape = tf.shape(input)
            N, H, W, C = shape[0], shape[1], shape[2], shape[3]
            input = tf.reshape(input, [N * H * W, C])
    ...
    @layer
    def incept(self, input, name):
        with tf.variable_scope(name) as scope:
            return tf.concat([input[0], input[1], input[2]], 3)
    ...
    def spatial_softmax(self, input, name):
        """ reshape成2类, 求softmax后, reshape回原样 """
        input_shape = tf.shape(input)
        # d = input.get_shape()[-1]
    ...
    def build_loss(self):
        ...
        rpn_cls_score = tf.gather(rpn_cls_score, rpn_keep) # 收集 shape (N, 2)
