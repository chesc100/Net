...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    post_nms_topN = cfg[cfg_key].RPN_POST_NMS_TOP_N     # 1000，做完nms之后，最多保留的box的数目
    ...
    keep = nms(np.hstack((proposals, scores)), nms_thresh)  #进行nms操作，保留1000个proposal
    if post_nms_topN > 0:
